...
3. **Test the PDF chatbot:**
   - Open http://localhost:8000/docs for API documentation
   - Upload PDFs: `POST /rag/upload-pdfs`
   - Chat with PDFs: `POST /rag/chat`
   
   ```bash
   # Upload PDF
   curl -X POST http://localhost:8000/rag/upload-pdfs \
     -F "files=@document.pdf"
   
   # Chat with PDF
   curl -X POST http://localhost:8000/rag/chat \
     -H 'Content-Type: application/json' \
     -d '{"message": "What is the main topic?", "session_id": "test"}'
   ```
